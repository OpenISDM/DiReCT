...
namespace DiReCT.DiReCTCore
{
    class DiReCTMainProgram : Application
    {
        ...
        private static Thread[] moduleThreadHandles 
            = new Thread[(int)ThreadRequiredModule.NumberOfModules];
        private static Thread UIThreadHandle;
        private static ModuleControlData[] modulesControlData 
            = new ModuleControlData[(int)ThreadRequiredModule.NumberOfModules];
        private static AutoResetEvent[] moduleReadyEvents 
            = new AutoResetEvent[(int)ThreadRequiredModule.NumberOfModules];
        private static bool 
            notInitializationTimeout = true; // Whether initialization processes
        ...
        static void Main()
        {
            ...
            moduleThreadHandles[(int)ThreadRequiredModule.AAA] 
                = new Thread(AAAModule.AAAInit);
            moduleThreadHandles[(int)ThreadRequiredModule.DM] 
                = new Thread(DMModule.DMInit);
            moduleThreadHandles[(int)ThreadRequiredModule.DS]
                = new Thread(DSModule.DSInit);
            moduleThreadHandles[(int)ThreadRequiredModule.MAN]
                = new Thread(MANModule.MANInit);
            moduleThreadHandles[(int)ThreadRequiredModule.RTQC]
            ...
            {
                moduleThreadHandles[(int)ThreadRequiredModule.AAA]
                    .Start(modulesControlData[(int)ThreadRequiredModule.AAA]
                    .ThreadParameters);
                moduleThreadHandles[(int)ThreadRequiredModule.DM]
                    .Start(modulesControlData[(int)ThreadRequiredModule.DM]
                    .ThreadParameters);
                moduleThreadHandles[(int)ThreadRequiredModule.DS]
                    .Start(modulesControlData[(int)ThreadRequiredModule.DS]
                    .ThreadParameters);
                moduleThreadHandles[(int)ThreadRequiredModule.MAN]
                    .Start(modulesControlData[(int)ThreadRequiredModule.MAN]
                    .ThreadParameters);
                moduleThreadHandles[(int)ThreadRequiredModule.RTQC]
                    .Start(modulesControlData[(int)ThreadRequiredModule.RTQC]
                    .ThreadParameters);
            }
            ...
            for (int i = 0; i < (int)ThreadRequiredModule.NumberOfModules; i++)
            {
                moduleReadyEvents[i] = modulesControlData[i].ThreadParameters
                                                            .ReadyToWorkEvent;
            }

            Timer timerOfIntialization 
                = new Timer(new TimerCallback(InitializationTimeOutEventHandler),
                            null,
            ...
            while (notInitializationTimeout)
            {
                if (WaitHandle.WaitAll(moduleReadyEvents, 
                                       (int)TimeInterval.VeryShortTime, 
                                       true))
                {
                    Debug.WriteLine(
